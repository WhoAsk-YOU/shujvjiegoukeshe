...
private slots:
    vector<DiaryInfo> getDiaries();  //获取查询结果
    void showDiaries();  //展示查询结果
    void tableClicked(int row, int column);  //若点击了表格中的第一列，则进入读日记界面
    void clickWrite();  //进入写日记界面

private:
    int mode = 1;  //若点击了目的地查询按钮,mode=1;若点击了日记名查询按钮,mode=2
    int page = 1;  //日记展示页数
    QString accountNumber;  //当前账户名
    QString place;  //当前所在景区/学校
    vector<DiaryInfo> diaries;  //符合要求的日记信息
    ...
    QPushButton* buttonNextPage = NULL;  //下一页按钮
    QPushButton* buttonPreviousPage = NULL;  //上一页按钮
    ...
    vector<DiaryInfo> sort(vector<DiaryInfo> diaries);
    void insertionSort(vector<DiaryInfo>& diaries);  //数据量较少，采用插入排序
